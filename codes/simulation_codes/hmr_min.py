...
hypo_method = os.environ.get("PSMIN_HYPO_METHOD", hypo_method).upper()
...
#   omega_HM,k = kappa ky / (1 + k^2)
#   omega_k = omega_HM,k + i (kappa/C) ky k^2 / (1 + k^2)^3
#   R_k = (C - i kappa ky) / (C - i omega_k)
#   n_k = R_k phi_k
omega_hm_k = kap * ky / (1.0 + ksqr)
omega_k = omega_hm_k + 1j * (kap / C) * ky * ksqr / (1.0 + ksqr) ** 3
r_k = (C - 1j * kap * ky) / (C - 1j * omega_k)
...
#   R_k = r_k
#   p_k = k^2 + R_k
# The nu_l coefficient is calibrated so nu_l/kmin^hypo_power = nu*kmax^2.
